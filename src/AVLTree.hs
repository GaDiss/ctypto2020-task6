...
instance (Show key, Show value) => Show (AVLTree key value) where
  ...
  show (LabelLeaf k _) = "L* " ++ show k
  show (Leaf k v) = "L " ++ show k ++ " " ++ show v
  show (Node h k _ l r) = "N "
                         ++ show h ++ " "
                         ++ show k  ++ " "
                         ++ "(" ++ show l ++ ") "
                         ++ "(" ++ show r ++ ")"
...
modify op (Leaf lk lv)
  | k == lk = case op of
                (Lookup _)    -> (Just lv, S.empty, Leaf lk lv)
                (Replace _ v) -> (Just v, S.empty, Leaf k v)
                (Insert _ _)  -> (Nothing, S.empty, Leaf lk lv)
                _             -> error "tree is incorrect"
  | k < lk = (Nothing, S.empty, Leaf lk lv)
  | k > lk = case op of
               (Insert _ v) -> (Just v, S.empty, newNode k (Leaf lk lv) (Leaf k v))
               _            -> (Nothing, S.empty, Leaf lk lv)
  where
    k = getKey op
modify _ _ = error "modify failed"
...
fromList = foldr (\ (k, v) t -> third3 $ doInsert k v t) MinLeaf
...
rotateL _ = error "failed to perform L rotation"
...
rotateLR _ = error "failed to perform LR rotation"
...
rotateR _ = error "failed to perform R rotation"
...
rotateRL _ = error "failed to perform RL rotation"

-- | compares heights of kids of a given node
compareKidsHeight :: AVLTree key value -> Ordering
compareKidsHeight (Node _ _ _ l r) = compare (height l) (height r)
...
replaceLeftLeaf leaf (LabelLeaf _ _)    = (Nothing, S.empty, leaf)
...
deleteRight _ = error "(impossible) failed to delete element in deleteRight"
